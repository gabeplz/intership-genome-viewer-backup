...
public class RulerPanel extends JPanel implements PropertyChangeListener{
	...
	public void paintComponent(Graphics g) {
		...
		for (int j = first; j < stop; j+= stepSize){
			...
			int[] info = DrawingTools.calculateLetterPosition(this.getWidth(), length,Double.valueOf(j-start)); //schaald de posities in sequentie naar de breedte van de panel
			int pos = info[1];
			g.drawLine(pos,40,pos,30);								// tekend lijn op de ruler
			g.drawString(String.valueOf(j + 1) + "bp", pos, 30);		// tekend de nucleotide positie in sequentie boven de lijn
